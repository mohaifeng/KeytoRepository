...
//日志输出函数
void MainWindow::LogMsgPrint(const QString &text)
{
    log->LogPrint(ui->LogplainTextEdit,text,Log::INFO);
}
...
void MainWindow::TxMsgPrint(const QString &msg)
{
    log->LogPrint(ui->LogplainTextEdit,msg,Log::TX);
}

void MainWindow::RxMsgPrint(const QString &msg)
{
    log->LogPrint(ui->LogplainTextEdit,msg,Log::RX);
}
...
void MainWindow::on_serOpenpushButton_clicked()
{
    if(ui->serOpenpushButton->text()=="打开串口")
    {
        if(pser->OpenPort(serialconfig))
        {
            ...
            LogMsgPrint(QString("%1打开成功，波特率:%2").arg(serialconfig.portName,QString::number(serialconfig.baudRate)));
        }
        else
        {
    ...
    {
        pser->ClosePort();
        LogMsgPrint(QString("%1关闭成功").arg(serialconfig.portName));
...
void MainWindow::on_ClearLogpushButton_clicked()
{
    ...
    UserRegListAppend(3,"dd","R","UL",12);
    UserNodeAppend(1,"ADP16");
}

void MainWindow::UserRegListAppend(int addr ,const QString &func,const QString &rw,const QString &unit,int value)
{
    // 获取当前行数
...
{
    QByteArray sendmsg = moem->MoemSenddataConfig(++nodeaddr,0,'k',"1Rp0");
