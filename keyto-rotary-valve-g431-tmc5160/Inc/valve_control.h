#ifndef __VALVE_CONTROL_H__
#define __VALVE_CONTROL_H__
...
#define VALVE_CONTROL_TASK_TIME 5
...
//#define DEFAULT_VALVE_MID_POSITION (DEFAULT_VALVE_MAX_POSITION-DEFAULT_VALVE_MIN_POSITION+1)/2     //Valve中间地址   
#define VALVE_TASK_TIME    50     //Valve任务间隔时间
...
typedef struct
{
  ...
  uint8_t CurrentOptocoupler2Cont;   		//当前位置光耦触发计数
	...
	/**********************/
  bool autoZeroMode;                //自动归零模式
  bool ZeroMarkFlag;								//已经归零标志
  bool AgingTestMode;               //老化测试模式
...
void Valve_Set_Mode(uint8_t mode);
...
int32_t uSetpToEncoderPosition(int32_t ustep);
int32_t EncoderTouSetpPosition(int32_t cont);
void Encoder_MotorRunTask(void);
