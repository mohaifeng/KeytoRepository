...
class SER_PROT_Trans:
    def __init__(self):
        ...
        self.send_flag = '3E'  # dt发送标志
        self.receive_flag = '3C'  # dt接收标志
        self.data_flag = ':'  # 数据标志
    ...
    # DT协议转OEM协议
    def DtToOem_Old(self, dt_data: str):
        ...
        oem_command_hex = oem_data.upper() + and_verification_hex
        return oem_command_hex

    def DtToOem_New(self, dt_data: str):
        oem_index = self.Oem_Index()
        if self.send_flag in dt_data:
            self.addr = int(a2b_hex(dt_data.split(self.send_flag)[0]).decode())
            addr_hex = hex(self.addr).replace('0x', '').zfill(2)  # 获取ADP地址
            self.data_hex = dt_data.split(self.send_flag)[1].replace('0D', '')  # 获取数据
            self.data_len = len(self.data_hex) // 2
            data_len_hex = hex(self.data_len).replace('0x', '').zfill(2)
            oem_data = self.send_head + oem_index + addr_hex + data_len_hex + self.data_hex
        elif self.receive_flag in dt_data:
            self.addr = int(dt_data.split(self.receive_flag)[0], 16)
            addr_hex = hex(self.addr).replace('0x', '').zfill(2)
            if self.data_flag in dt_data:
                ...
                self.data_len = len(self.data_hex) // 2
                data_len_hex = hex(self.data_len).replace('0x', '').zfill(2)
                oem_data = self.receive_head + oem_index + addr_hex + adp_state_hex + data_len_hex + self.data_hex
            else:
                self.data_len = 0
                data_len_hex = hex(self.data_len).replace('0x', '').zfill(2)
                adp_state = int(dt_data.split(self.receive_flag)[1])  # 获取数据
                adp_state_hex = hex(adp_state).replace('0x', '').zfill(2)
                oem_data = self.receive_head + oem_index + addr_hex + adp_state_hex + data_len_hex
        else:
            print('数据格式错误！:', dt_data)
            return 0
        and_verification_hex = cs.Uchar_Checksum_8_Bit(oem_data)
    ...
    def OemToDt(self, oem_str: str):
        ...
        if oem_str[0:2] == self.send_head:
            self.data_len = int(oem_str[4:6], 16)
...
if __name__ == '__main__':
    ...
    print(ser_p.DtToOem_Old('1>It500,100,0'))
    print(ser_p.DtToOem_New('1>It500,100,0'))
